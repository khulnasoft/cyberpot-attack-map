...
window.processRestoredAttack = function (event, isPlayback = false) {
    ...
    const restoredMsg = {
        ...
        cyberpot_hostname: event.cyberpot_hostname || event.honeypot || 'honeypot',
...
function restoreMarkerData(restoredMsg, srcLatLng, dstLatLng, originalEvent, isPlayback = false) {
    ...
    addMarker(restoredMsg.dst_country_name, restoredMsg.dst_iso_code,
        restoredMsg.dst_ip, restoredMsg.cyberpot_hostname, dstLatLng);
}
...
function addMarker(dst_country_name, dst_iso_code, dst_ip, cyberpot_hostname, dstLatLng) {
    // Validate parameters
...
function createHoneypotPopup(honeypotData) {
    ...
    const h4 = document.createElement('h4');
    h4.textContent = 'CyberPot Honeypot';
    ...
    Traffic: (msg) => {
        ...
        if (!markerAttackData[dstKey]) {
            markerAttackData[dstKey] = {
                ...
                dst_ip: msg.dst_ip,
                hostname: msg.cyberpot_hostname,
                attacks: [],
                totalAttacks: 0,
                uniqueAttackers: new Set(),
                protocolStats: {},
        ...
        Promise.all([
            ...
            addMarker(msg.dst_country_name, msg.dst_iso_code, msg.dst_ip, msg.cyberpot_hostname, dstLatLng),
        ...
        if (window.attackMapDashboard) {
            const attackData = {
                ...
                src_ip: msg.src_ip,
                ip_rep: msg.ip_rep,
                cyberpot_hostname: msg.cyberpot_hostname,
                color: msg.color,
                country: msg.country,
                country_code: msg.iso_code,
                iso_code: msg.iso_code,
                protocol: msg.protocol,
                honeypot: msg.honeypot, // Use honeypot field from message, not cyberpot_hostname
                port: msg.dst_port,
